...
/**
 * Check if an image is exactly 1080px wide (edge case for responsive images)
 * Images exactly 1080px wide only generate -1080w versions, requiring special handling
 *
 * @param {string} filePath - Path to image file
 * @returns {Promise<object>} - { is1080px: boolean, width: number|null, height: number|null }
 */
async function checkImageWidth(filePath) {
    const sharp = require('sharp');

    try {
        const metadata = await sharp(filePath).metadata();
        return {
            is1080px: metadata.width === 1080,
